...
# adopted from TF-MODISCO
# make All or some PPM columns add to 1
def normalize_pwm(pwm_list):
    for i, pwm in enumerate(pwm_list):
        pwm_list[i] = pwm / pwm.sum(axis=0)
    return pwm_list
...
def _ic_scale(pwm, background=[0.25, 0.25, 0.25, 0.25]):
...
def plot_weights(array, figsize=(20, 2), ic_scale=False, **kwargs):
    fig = plt.figure(figsize=figsize)

    ax = fig.add_subplot(111)
    if ic_scale:
        array = _ic_scale(array)
        plot_weights_given_ax(ax=ax, array=array, height_range=[0, 2], **kwargs)
    else:
