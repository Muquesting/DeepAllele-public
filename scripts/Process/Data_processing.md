...
Alternatively, can insert variants individually into region FASTAs from a vcf file as follows in insert_variants.py


First we can get the fasta file corresponding to peaks in a bed file using bedtools:
```
bedtools getfasta -name -fi fasta_name.fa -bed peaks.bed -fo region_fasta.fa

```

Then we can intersect the snp/indel vcf's with the peak coordinates (using the version of the bed file without "chr", e.g.
_nochr.bed)

```
bedtools intersect -wo -a snps.vcf -b peaks_nochr.bed > snp_peak_intersect.txt

bedtools intersect -wo -a indels.vcf -b peaks_nochr.bed > indel_peak_intersect.txt

```

Then we can insert variants into the region fasta:
...
```
# ${bam}.fragments.sort.bed.gz is the processed version of the fragments file generated by sinto (${bam}.fragments.bed) 
