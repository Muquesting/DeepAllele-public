...
def train_model(
    ...
    # single_model = trainer.load_from_checkpoint(
    ...
    tools.write_meme_file(pwm, checkpoint_path + "/selected_motifs_transfer.meme")
    tools.write_meme_file(PWM, checkpoint_path + "/all_motifs_transfer.meme")

    single_model.activation = True
    all_predict_y_activation = trainer.predict(single_model, valloader)
    activations = torch.concat(all_predict_y_activation).numpy()

    # get the input sequences from the validation dataloader
    seqs_list = []
    labels_list = []
    ...
    seqs = torch.cat(seqs_list).numpy()

    PWM, n_activations = tools.extract_motifs_activation_seqs(
        activations,
        seqs,
        motif_length=20,
        kernel_length=15,
        activation_threshold=0.5,
        multiple_genome=True,
    )
    ...
    tools.write_meme_file(pwm, checkpoint_path + "/selected_motifs_activation.meme")
    tools.write_meme_file(PWM, checkpoint_path + "/all_motifs_activation.meme")
...
if __name__ == "__main__":
    ...
    trainloader, valloader, _,_ = data.load_h5(in_folder, 0.9, 32, batch_id=batch_id, split_by_chrom=True)
