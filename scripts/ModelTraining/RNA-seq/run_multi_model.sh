...
OUT_FOLDER="./output/"
PROJECT_ID="DeepAllele-F1-mouse-RNA-reproduce"  # Project ID for multi-head runs
...
# Checkpoints will be saved in the following hierarchy:
# <OUT_FOLDER>/<hyper parameters>/<batch_{BATCH_ID}>/random_seed_{seed}/checkpoint
python reproduce_multi_head_models.py \
  --mode train \
  --in_folder "$IN_FOLDER" \
...
# To run validation with a saved checkpoint, adjust checkpoint path accordingly.
